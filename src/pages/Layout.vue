  ...
  <el-container direction="vertical" class="management-layout-container">
    <el-header class="layout-header" style="height: auto;">
      <div class="layout-header-title">
        <div class="logo">
          <h2 class="h2" @click="handleHome">XXXX后台管理系统</h2>
        </div>
        <div class="account">
          <div class="user">
              ...
              >
                <template slot="title">
                  <div @click.stop="routerTo(item)" class="sub-menu-parent-container">
                    <img
                      :src="'/static/menu-icon/' + item.icon + '.png'"
                      class="icon icon-no-active"
                      v-if="item.icon"
                    />
                    <img
                      :src="'/static/menu-icon/' + item.icon + '_active.png'"
  ...
  methods: {
    ...
    routerTo(item) {
      if (
        item.items &&
        item.items.length &&
        this.$route.path !== item.items[0].path
      ) {
    ...
    handleHome() {
      if (this.$route.path !== "/dashboard") this.$router.replace("/dashboard");
    }
  },
